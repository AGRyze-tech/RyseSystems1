...
function buildWhatsAppURL(data: {
  ...
  const msg = [
    `Olá! Vim pelo site da RyzeSystems e gostaria de solicitar um diagnóstico gratuito.`,
    ``,
    `*Nome:* ${data.name}`,
    `*WhatsApp:* ${data.whatsapp}`,
    `*Faturamento mensal:* ${data.revenue}`,
    `*Pacientes/clientes por mês:* ${data.patientsPerMonth}`,
    `*Já tem site?* ${data.hasSite}`,
  ].join('\n')
...
export default function Contato() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const data = {
      ...
      patientsPerMonth: (fd.get('patientsPerMonth') as string) ?? '',
      hasSite: (fd.get('hasSite') as string) ?? '',
    }

    // 1. Abre WhatsApp imediatamente com os dados preenchidos
    const waURL = buildWhatsAppURL(data)
    window.open(waURL, '_blank', 'noopener,noreferrer')

    // 2. Tenta enviar email via API (não bloqueia a experiência)
    try {
      ...
      // silencioso — o WhatsApp já foi aberto
    }
  ...
  return (
    <section id="contato" className="bg-ryze-dark relative px-6 py-16 lg:px-8 lg:py-36">
      <motion.div
        layout={false}
        initial="hidden"
        whileInView="visible"
      ...
      >
        <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr] lg:gap-20">
          ...
          {/* Left — info */}
          <motion.div variants={fadeUp} transition={smoothTransition} className="lg:pt-10">
            ...
            {status === 'success' ? (
              ...
              >
                ...
                <p className="text-white/60">
                  O WhatsApp foi aberto com suas informações. Entraremos em contato em breve!
                </p>
                <button
                  type="button"
                  onClick={() => setStatus('idle')}
                  className="mt-4 text-sm font-medium text-ryze-cta hover:text-ryze-accent transition-colors"
                >
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <div>
                  <label htmlFor="revenue" className="mb-2 block text-sm font-medium text-white/60">
                    Faturamento mensal aproximado *
                  </label>
                  <input
                    id="revenue" name="revenue" type="text" required
                    inputMode="text" placeholder="Ex: R$10.000 / mês"
                    className={inputClass}
                  />
                </div>

                <div>
                  <label htmlFor="patientsPerMonth" className="mb-2 block text-sm font-medium text-white/60">
                    Clientes / pacientes por mês *
                  </label>
                  <input
                    id="patientsPerMonth" name="patientsPerMonth" type="text" required
                    inputMode="numeric" placeholder="Ex: 40 pacientes por mês"
                    className={inputClass}
                  />
                </div>
                ...
                {status === 'error' && (
                  <motion.div
                    ...
                    className="flex items-center gap-2 rounded-xl border border-red-500/20 bg-red-500/[0.06] px-4 py-3 text-sm text-red-400"
                  >
